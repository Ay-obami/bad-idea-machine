...
5. The five `POSSIBLE OUTCOMES` cards use the same photographic aftermath assets that can appear in the main stage.
...
Each environment ships these local assets under `public/cinematic/<environment>/`:

- `idle.webp` — intact room.
- `chaos.webp` — authored live-chaos plate used during reveal.
- `result-0.webp` through `result-4.webp` — persistent multiplier aftermaths.
- `after-heavy.webp` — optional transition texture for high-intensity reveal moments.
...
- all five outcome cards point to distinct `result-0`…`result-4` room assets;
