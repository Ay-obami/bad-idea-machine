...
Run `npm run report-kitchen-terminal-readiness` from the Kitchen example directory for a per-tier file count, or add `-- --details` for every missing path. `-- --strict` exits unsuccessfully while any planned terminal source is absent. The normal development build remains available during art production. The first audit counted 160 symbolic paths, including files for Tier 1 no-fire/no-smoke/normal-power/no-cabinet-debris and Tier 2 normal-power. Empty states now produce no asset requirement. Identical states across tiers share the same proposed source path. The revised plan contains **155 tier references to 111 unique source paths** (Tier 1 has 28 references), and none of those terminal source paths is authored yet. The local `rebuild/truth` assets and the Tier 1/Tier 2 visual studies are real files used by the stationary proof; [the Tier 1 map](tier1-asset-map.md) identifies which can be reused and which need art. Plan-shape tests and a passing build therefore do not satisfy the static aftermath gate.
...
The readiness report now lists repository-local evidence separately from the missing terminal paths: directly reusable truth frames, sources requiring placement or zone composition, and provisional visual studies. It does not mark those references as completed terminal art. Tier 1 currently maps 9 direct, 14 composed and 5 study references. The five studies include repeated ownership of one physical shattered hero plate and debris field; they do not imply five finished assets. Tier 2 maps 4 composed and 5 study references, with 22 unmapped. All 111 unique proposed terminal paths remain missing, so `--strict` still fails. The separate upper-cabinet soot-edge trial passed only a bounded attachment and clean-hide check.

The older `kitchen-layered-static` and `kitchen-aftermath-static` review URLs now route to the repository-local approved-master stationary proof (intact and Tier 1 respectively). Their old remote-atlas components are no longer imported into the production bundle; historical source files remain for provenance only. This prevents those review URLs from displaying an older Kitchen atlas while the final art gate is incomplete. It does not change the existing game sequence or authorize animation.
