...
import {
  ...
  routerInitAbi,
  routerInitBytecode,
  trustedOperatorEnclaveKeyVerifierAbi,
  trustedOperatorEnclaveKeyVerifierBytecode,
  vrfFacetAbi,
  vrfFacetBytecode,
} from './artifacts.ts';
...
// balance and node writes, the events, and the echoed callback data as calldata (about 17 gas a
// byte). EIP-150 keeps 1/64 of the gas back from the callback's call, so the router requires
// 64/63 of the callback gas limit to be left when it makes it.
const FULFILLMENT_GAS_BESIDES_CALLBACK = 150_000n;
...
export const randomnessRequestedEvent = parseAbiItem(
  'event RandomnessRequested(bytes32 indexed requestId, address indexed consumer, address indexed fulfiller, (address consumer, uint64 sequence, address fulfiller, uint40 assignedAt, uint96 fee, uint32 callbackGasLimit, bytes callbackData) request)',
);
...
/**
 * Deploys the router diamond (its facets, the trusted-operator attestation verifier, and the
 * ECVRF verifier challenges run), trusts the node's wallet as an operator, and registers the
 * node with the router's stake. Returns the router address.
 */
export async function setupLocalVerifyNetwork({
  ...
  const trustHash = await walletClient.writeContract({
    abi: trustedOperatorEnclaveKeyVerifierAbi,
    address: attestationVerifierAddress,
    functionName: 'setTrustedOperator',
    args: [nodeAccount.address, true],
  });
